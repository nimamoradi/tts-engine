...
class Synthesizer:
	def load(self, checkpoint_path, hparams, gta=False, model_name='Tacotron'):
		...
		with tf.variable_scope('Tacotron_model') as scope:
			...
			if gta:
				self.model.initialize(inputs, input_lengths, targets,
				                      gta=gta, split_infos=split_infos)
			else:
	...
	def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames):
		...
		if self.gta or not hparams.predict_linear:
			...
			linears, mels, alignments, stop_tokens = self.session.run(
			    [self.linear_outputs, self.mel_outputs, self.alignments, self.stop_token_prediction], feed_dict=feed_dict)
			# Linearize outputs (1D arrays)
		...
		for i, mel in enumerate(mels):
			...
			if log_dir is not None:
				...
				audio.save_wav(wav, os.path.join(
				    log_dir, 'wavs/wav-{}-mel.wav'.format(basenames[i])), sr=hparams.sample_rate)
