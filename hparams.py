...
hparams = tf.contrib.training.HParams(
	...
	hop_size = 138, #For 22050Hz, 275 ~= 12.5 ms (0.0125 * sample_rate)
	win_size = 550, #For 22050Hz, 1100 ~= 50 ms (If None, win_size = n_fft) (0.05 * sample_rate)
	sample_rate = 11025, #22050 Hz (corresponding to ljspeech dataset) (sox --i <filename>)
	...
	tacotron_batch_size = 56, #number of training samples on each training steps
	...
	#performance parameters
	wavenet_swap_with_cpu = True, #Whether to use cpu as support to gpu for synthesis computation (while loop).(Not recommended: may cause major slowdowns! Only use when critical!)

	#train/test split ratios, mini-batches sizes
	wavenet_batch_size = 56, #batch size used to train wavenet.
	...
	u"آگاه باشید این ها همان مفسد ولی نمی‌ فهمند",
	"آگاه باشید این ها همان مفسد ولی نمی‌ فهمند",
	"دنمهف‌یمن یلو دسفم نامه اه نیا دیشاب هاگآ"	,
	u"دنمهف‌یمن یلو دسفم نامه اه نیا دیشاب هاگآ"	
